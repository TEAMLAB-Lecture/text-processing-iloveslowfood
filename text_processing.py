...
def normalize(input_string: str) -> str:
    ...
    * 앞뒤 필요없는 띄어쓰기는 제거해야함
    ...
    normalized_string_parsed = list(
        map(lambda x: x.lower(), filter(lambda x: len(x) > 0, parsed_string))
    )
    normalized_string = " ".join(normalized_string_parsed)
...
def no_vowels(input_string):
    ...
    for v in vowel:
        if v in no_vowel_string:
            no_vowel_string = no_vowel_string.replace(v, NONE)
